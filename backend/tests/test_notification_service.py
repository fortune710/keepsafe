...
async def test_send_notification_rate_limit_retry(notification_service):
    ...
    # Arrange - Mock httpx for REST API with rate limit then success
    import httpx
    ...
    success_response.json.return_value = [{"status": "ok", "id": "test-id"}]
...
async def test_send_notification_max_retries_exceeded(notification_service):
    ...
    # Arrange - Mock httpx for REST API with rate limit error
    import httpx
    from unittest.mock import AsyncMock, patch
    ...
    with patch("httpx.AsyncClient") as mock_client_class:
        ...
        mock_client.__aenter__.return_value = mock_client
        mock_client.__aexit__.return_value = None
        mock_client.post = AsyncMock(side_effect=rate_limit_error)
        mock_client_class.return_value = mock_client
