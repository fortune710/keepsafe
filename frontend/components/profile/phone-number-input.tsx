...
export function PhoneNumberInput({
  ...
  useEffect(() => {
      ...
      if (!hasUserInput) {
        if (defaultCountryIso) {
          const country = countries.find(
            (c) => c.iso.toUpperCase() === defaultCountryIso.toUpperCase(),
          );
          if (country) {
    ...
    if (!hasUserInput) {
      ...
      const matchingCountries = countries.filter((c) =>
        initialValue.startsWith(c.code),
      );
      ...
      if (longestMatch) {
        ...
        setValue(
          formatPhoneNumber(
            initialValue.slice(longestMatch.code.length).trim(),
          ),
        );
        return;
      }
  ...
  const emitPhoneState = (
    ...
    const currentIsValid =
      currentNormalized.length > 0 && phoneRegex.test(currentNormalized);
